...
def test(strategy, model, log_file='test_pacman_log.json', render=False):
...
def random_strategy(obs, model):
    return random.choice(obs['possible_actions'])


def naive_strategy(obs, model):
    best_dist = 0
    best_act = 4
